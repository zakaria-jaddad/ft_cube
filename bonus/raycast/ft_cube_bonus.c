...
/*   Updated: 2025/11/08 20:48:28 by zajaddad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_hand(t_game *game) {
  mlx_texture_t *zlije = mlx_load_png("./bonus/textures/final_zlayji_pov.png");
  if (zlije == NULL)
    return;

  for (int x = 0; x < (int)zlije->width; x++) {
    for (int y = 0; y < (int)zlije->height; y++) {
      uint32_t color = sample_texture_rgba(zlije, x, y);
      if (color == (uint32_t)0)
        continue;
      color = shade_rgba(color, 0.75);
      mlx_put_pixel(game->mlx->img, x, y, color);
